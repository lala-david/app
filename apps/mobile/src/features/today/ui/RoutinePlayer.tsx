...
import { YoutubePlayer } from '@/shared/platform/youtube/YoutubePlayer';
...
const COVER_TEXT: Record<Exclude<PlayerPhase, 'playing'>, string> = {
 ...
 */
export function RoutinePlayer({ routine, phase, wantPlay, onState, onError, onPlay, onOpenOutside }: Props) {
  ...
  return (
    <Appear style={[styles.frame, { borderColor: tone.p }]}>
      {blocked ? null : <YoutubePlayer source={routine.video} playing={wantPlay} onState={onState} onError={onError} />}

      {phase === 'playing' ? null : (
        <Pressy onPress={onPlay} disabled={!tappable} pressedScale={1} style={[styles.cover, { backgroundColor: tone.p }]} accessibilityLabel={COVER_TEXT[phase]}>
